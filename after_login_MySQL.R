...
body <- dashboardBody(
    tabItems(
        ...
        tabItem(tabName = "my_room",
                # header
                fluidRow(box(width=5,height = 15,
                             h4("UCL Statistical Science Department Room Use System"), 
...
server <- shinyServer(function(input,output,session){
    ...
    update_status <- function(room_no,date,use="write",am=NA,avail=NA,database,table) {
        ...
        if(use=="write"){
            ...
            q=paste(query_am,query_pm,query_both,query_neither,sep=",")
