...
#
create_next_month <- function(name,
                              database,
                              table){
  ...
  for(i in 0:30){
    ...
    ##      is this a truthy thing?
    
    isWeekend <- weekday_to_cha(Sys.Date() + i) %in% c('Sat','Sun') ##TODO: replace below
    
    if(sum(room_data$Name == name &
  ...
  create(name = name_vec,
         date = date_vec,
         day_of_the_week = day_vec,
         available_period = NA,
         notes = NA,
         database = database,
         table = table)
...
# con <- dbConnect(RMySQL::MySQL(), dbname = "room_avail")
# dbGetInfo(con)
...
create(
  ...
  'Mon',
  'both',
  'first let available period take value both',
  database = 'room_avail',
  table = 'room_table'
)
loadData('room_avail','room_table')
...
create(
  ...
  'Mon',
  'neither',
  'then change it to neither with the same function',
  database = 'room_avail',
  table = 'room_table'
)
loadData('room_avail','room_table')
...
create_next_month(name = 'Richard',
                  database = 'room_avail',
                  table = 'room_table') #Take a few seconds
loadData('room_avail','room_table')
...
delete('Richard',
       ...
       '2019-08-28',
       database = 'room_avail',
       table = 'room_table')#Delete all rows between two dates
loadData('room_avail','room_table')
